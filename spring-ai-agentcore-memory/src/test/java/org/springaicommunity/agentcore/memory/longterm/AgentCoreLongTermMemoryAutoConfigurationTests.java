...
class AgentCoreLongTermMemoryAutoConfigurationTests {
	...
	void shouldCreateAllAdvisorsWhenAllConfigured() {
		this.contextRunner.withUserConfiguration(MockClientConfiguration.class)
			...
			.run((context) -> {
				...
				Map<String, AgentCoreLongTermMemoryAdvisor> advisors = context
	...
	void shouldUseCustomNamespacePattern() {
		...
		this.contextRunner.withUserConfiguration(CustomNamespaceConfiguration.class)
			.withPropertyValues(MEMORY_ID_PROP, SEMANTIC_STRATEGY_PROP,
	...
	void shouldCollectAllAdvisorsViaListInjection() {
		this.contextRunner.withUserConfiguration(MockClientConfiguration.class)
			...
			.run((context) -> {
				...
				Collection<AgentCoreLongTermMemoryAdvisor> advisors = context
					.getBeansOfType(AgentCoreLongTermMemoryAdvisor.class)
					.values();
				assertThat(advisors).hasSize(4);
	...
	void autodiscoveryShouldReturnEmptyWhenNoStrategies() {
		this.contextRunner.withUserConfiguration(EmptyDiscoveryMockConfiguration.class)
			.withPropertyValues(MEMORY_ID_PROP, "agentcore.memory.long-term.auto-discovery=true")
			.run((context) -> {
				assertThat(context).hasNotFailed();
	...
	void autodiscoveryShouldSkipCustomStrategyType() {
		this.contextRunner.withUserConfiguration(AutodiscoveryWithCustomMockConfiguration.class)
			.withPropertyValues(MEMORY_ID_PROP, "agentcore.memory.long-term.auto-discovery=true")
			.run((context) -> {
				assertThat(context).hasNotFailed();
	...
	void autodiscoveryShouldUseFirstNamespace() {
		this.contextRunner.withUserConfiguration(MultipleNamespacesMockConfiguration.class)
			.withPropertyValues(MEMORY_ID_PROP, "agentcore.memory.long-term.auto-discovery=true")
			.run((context) -> {
				assertThat(context).hasNotFailed();
