...
class AgentCoreIntegrationTests {
	...
	void shouldHandleInvocationsEndpoint() {
		...
		var response = this.restTemplate.postForEntity("http://localhost:" + this.port + "/invocations", request,
				Map.class);
	...
	void shouldHandlePingEndpoint() {
		var response = this.restTemplate.getForEntity("http://localhost:" + this.port + "/ping", Map.class);
