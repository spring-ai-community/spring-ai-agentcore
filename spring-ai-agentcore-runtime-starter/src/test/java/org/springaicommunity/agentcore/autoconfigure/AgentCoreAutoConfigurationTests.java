...
class AgentCoreAutoConfigurationTests {
	...
	void shouldAllowCustomObjectMapperOverride() {
		this.contextRunner.withUserConfiguration(CustomObjectMapperConfiguration.class).run((context) -> {
			assertThat(context).hasSingleBean(ObjectMapper.class);
	...
	void shouldAllowCustomRegistryOverride() {
		this.contextRunner.withUserConfiguration(CustomRegistryConfiguration.class).run((context) -> {
			assertThat(context).hasSingleBean(AgentCoreMethodRegistry.class);
	...
	static class CustomRegistryConfiguration {
		...
		@Bean
		AgentCoreMethodRegistry customRegistry() {
