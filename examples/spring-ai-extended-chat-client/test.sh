...
echo "   - Alice knows about Bob: $(echo "$ALICE_R4" | jq -r '.response' | grep -iE "(data scientist|microsoft|machine learning|rex)" > /dev/null && echo "❌ LEAKED" || echo "✅ ISOLATED")"
echo ""
...
echo "   - Memory Isolation: $(echo "$ALICE_R4" | jq -r '.response' | grep -i "bob\|microsoft\|rex" > /dev/null && echo "❌ FAILED" || echo "✅ SUCCESS")"
echo ""
