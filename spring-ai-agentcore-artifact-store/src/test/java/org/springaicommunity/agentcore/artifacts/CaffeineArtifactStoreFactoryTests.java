...
class CaffeineArtifactStoreFactoryTests {

	private ArtifactStoreFactory factory;

	@BeforeEach
	void setUp() {
		this.factory = new CaffeineArtifactStoreFactory();
	}

	@Test
	@DisplayName("Should create store with name only using defaults")
	void shouldCreateStoreWithNameOnlyUsingDefaults() {
		ArtifactStore<GeneratedFile> store = this.factory.create("TestStore");
	...
	void shouldCreateStoreWithCustomTtl() {
		ArtifactStore<GeneratedFile> store = this.factory.create("TestStore", 600);
	...
	void shouldCreateStoreWithFullConfiguration() {
		ArtifactStore<GeneratedFile> store = this.factory.create("TestStore", 600, 5000);
